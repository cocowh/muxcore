...
// ProcessorManager 处理器管理器接口
type ProcessorManager interface {
	SelectProcessor(protocol string) ProtocolHandler
	GetMetrics(protocol string) interface{}
}
...
func NewOptimizedProtocolProcessor(config *ProcessorConfig, processorMgr ProcessorManager) *OptimizedProtocolProcessor {
...
func (opp *OptimizedProtocolProcessor) processTask(task *ProcessingTask) {
	...
	defer func() {
		...
		// 记录性能指标
		// Performance monitoring removed to avoid circular dependency
	}()
