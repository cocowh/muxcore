...
// SetBaseline 设置基线值
func (ad *AnomalyDetector) SetBaseline(key string, value float64) {
	ad.mu.Lock()
	defer ad.mu.Unlock()
	ad.baselines[key] = value
}

// GetBaseline 获取基线值
func (ad *AnomalyDetector) GetBaseline(key string) (float64, bool) {
	ad.mu.RLock()
	defer ad.mu.RUnlock()
	value, exists := ad.baselines[key]
	return value, exists
}
...
// GetThreshold 获取阈值
func (ad *AnomalyDetector) GetThreshold() float64 {
	ad.mu.RLock()
	defer ad.mu.RUnlock()
	return ad.threshold
}
...
func (ad *AnomalyDetector) DetectAnomaly(key string, value float64) bool {
	...
	if !exists {
		...
		ad.mu.Lock()
		// 双重检查模式，防止并发设置基线
		if _, stillNotExists := ad.baselines[key]; stillNotExists {
			ad.baselines[key] = value
		}
		ad.mu.Unlock()
		return false
...
func (td *ThreatDetector) detectByBehavior(data []byte, source, target string) []*ThreatEvent {
	...
	// 在锁外计算相关值，避免在持锁期间调用其他方法
	requestRate := td.calculateRequestRate(source)
	avgRequestSize := td.calculateAvgRequestSize(source, len(data))

	// 重新获取锁更新画像
	td.mu.Lock()
	profile.LastUpdate = time.Now()
	profile.RequestRate = requestRate
	profile.AvgRequestSize = avgRequestSize
...
func (re *ResponseEngine) IsBlocked(ip string) bool {
	// 使用锁分离与双重检查，避免在持有读锁时解锁再升级为写锁导致的潜在死锁或重复解锁
	re.mu.RLock()
	...
	if time.Now().After(blockTime) {
		// 过期后在写锁下安全删除，采用双重检查以避免竞态
		re.mu.Lock()
		if bt, ok := re.blacklist[ip]; ok && time.Now().After(bt) {
...
func (sm *OptimizedSecurityManager) GetThreatSignatures() map[string]*ThreatSignature {
	...
	signatures := make(map[string]*ThreatSignature)
	maps.Copy(signatures, sm.threatDetector.signatures)
	return signatures
}
...
func (sm *OptimizedSecurityManager) GetBehaviorProfiles() map[string]*BehaviorProfile {
	...
	maps.Copy(profiles, sm.threatDetector.behaviorProfiles)
	return profiles
}
...
func (sm *OptimizedSecurityManager) GetBlockedIPs() map[string]time.Time {
	...
	maps.Copy(blockedIPs, sm.responseEngine.blacklist)
	return blockedIPs
}
